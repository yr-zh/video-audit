...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if "video" in content and isinstance(content["video"], dict):
            ...
            video_path = video_path.replace("/tmp/dataset/", "/mnt/data/leaderboard/32ca9448ba8c2b4e4409fd5edc5123a8/")
...
class AttentionFusion3d(nn.Module):
    ...
    def forward(self, v1, v2, v3, v4):
        ...
        stacked_vectors = torch.stack([v1, v2, v3, v4], dim=1)  # [batch_size, 3, 512]
        ...
        fused_vector = (
            ...
            attention_weights[:, 3].unsqueeze(-1) * v4
        )
        return fused_vector
...
class CLIPClassifier(nn.Module):
    ...
    def forward(self, texts, images, inputs):
        ...
        frame_embeddings = outputs.image_embeds
        text_embeddings = torch.from_numpy(self.text_model.encode(texts)).to('cuda')
        cover_embeddings = torch.from_numpy(self.img_model.encode(images)).to('cuda')
        # fused_vector_concat = torch.cat((tag_embeddings, text_embeddings, img_embeddings), dim=1)
        fused_vector = self.attention(frame_embeddings, cover_embeddings, tag_embeddings, text_embeddings)
...
def extract_frames(video_path: str, max_frames: int = 10) -> List[np.ndarray]:
    """
    从视频中每秒抽取1帧，最多抽取指定数量的帧
    
    Args:
        video_path: 视频文件路径
        max_frames: 最大抽取帧数（默认10帧）
    
    Returns:
        帧列表（BGR格式的numpy数组），如果失败返回空列表
    """
    frames = []
    try:
        # 打开视频文件
        ...
        # 确定要抽取的帧位置（时间点）
        interval = 1.0  # 每秒1帧
        target_times = [min(i, duration - 0.1) for i in np.arange(0, duration, interval)][:max_frames]
        ...
        for target_time in target_times:
            ...
            ret, frame = cap.read()
            if ret:
                frames.append(frame.copy())  # 避免引用问题
...
def train(rank, world_size):
    ...
    # checkpoint = "models_chinese_bi_01_2_final_shuffled/clip_finetuned_classifier_epoch1.pth"
    checkpoint = None
    ...
    # dataset = CustomImageDataset(file_path='datasets/oversea/index_train_final_shuffled.jsonl', transform=None)
    ...
    ckpt_path = "models_oversea_frame"
    if rank == 0:
    ...
    for epoch in range(num_epochs):
        ...
        for item in tqdm(dataloader):
            ...
            images = [load_image(i) for i in img_paths]
            first_frames = []
            for i in range(len(video_paths)):
                try:
                    first_frame = get_first_frame(video_paths[i])
