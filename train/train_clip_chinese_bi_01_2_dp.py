...
def main():
    ...
    dataset = CustomImageDataset(file_path='datasets/fine/index.jsonl', transform=None)
    ...
    optimizer = optim.Adam(model.module.classifier.parameters(), lr=1e-4)
    optimizer = nn.DataParallel(optimizer)

    # scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_epochs, eta_min=0)
    scheduler = optim.lr_scheduler.StepLR(optimizer.module, step_size=1, gamma=0.1)

    ckpt_path = "models_chinese_bi_01_2_0410"
    ...
    for epoch in range(num_epochs):
        print(print(optimizer.module.param_groups[0]["lr"]))
        model.train()
        running_loss = 0.0
        ...
        torch.save(model.module.state_dict(), f"{ckpt_path}/clip_finetuned_classifier_epoch{epoch+1}.pth")
