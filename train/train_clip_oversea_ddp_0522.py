...
class CLIPClassifier(nn.Module):
    def __init__(self, clip_model, bert_model, tokenizer, num_classes):
        """
    ...
    def forward(self, texts, images, inputs):
        ...
        text_embedding = self.bert_model(**tokens)
        import pdb
        pdb.set_trace()
        fused_vector = self.attention(img_embeddings, tag_embeddings, text_embedding)
...
def train(rank, world_size):
    ...
    model = CLIPClassifier(clip_model, bert_model, tokenizer, num_classes).to(device)
    if checkpoint:
